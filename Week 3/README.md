Nederlands

---

| [Week 2 - Kubernetes Networking & CI/CD](../Week%202/README.md) |
|:---|

---

# Week 3 - Blue-Green Deployments & Artifact Registry

## Onderwerpen

Deze week implementeren we een **Blue-Green deployment** voor een applicatie via **Google Artifact Registry** als alternatief voor Docker Hub. We koppelen een CI/CD-pipeline die automatisch een nieuw image bouwt en uitrolt bij elke codechanging.

---

## Leerdoelen

- Blue-Green deployment opzetten en begrijpen
- Google Artifact Registry gebruiken als container registry
- CI/CD pipeline inrichten met GitHub Actions voor deployment naar GKE
- Wisselen tussen Blue en Green versies via een Kubernetes Service
- Alternatieven voor GitHub Actions kennen: Argo CD en Flux CD

---

## Leermaterialen

| Resource | Link |
|---|---|
| Google Artifact Registry documentatie | [cloud.google.com](https://cloud.google.com/artifact-registry/docs) |
| CI/CD naar GKE met GitHub Actions | [medium.com](https://medium.com/@gravish316/setup-ci-cd-using-github-actions-to-deploy-to-google-kubernetes-engine-ef465a482fd) |
| Argo CD documentatie | [argo-cd.readthedocs.io](https://argo-cd.readthedocs.io/en/stable/) |
| Flux CD documentatie | [fluxcd.io](https://fluxcd.io/flux/) |

---

## Opdrachten & Uitwerkingen

| Map | Beschrijving |
|---|---|
| [Opdracht/](Opdracht/) | Opdrachten voor week 3 |
| [Uitwerking/](Uitwerking/) | Mijn uitwerkingen voor week 3 |

---

| [Week 2 - Kubernetes Networking \& CI/CD](../Week%202/README.md) |
|:---|
